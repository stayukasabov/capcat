...
class SourceRegistry:
    ...
    def __init__(
        ...
        Args:
            ...
            project_root: Project root for user source discovery.
                ...
                user sources come from <project_root>/Config/sources/active/.
        """
        ...
        # Builtin sources: inside the installed package
        # Path(__file__) = capcat/core/source_system/source_registry.py
        # .parent.parent.parent = capcat/
        self._builtin_path = (
            Path(__file__).parent.parent.parent / "sources" / "builtin"
        )

        # User sources: inside the project Config/
        if self._project_root is not None:
            self._user_path = (
                self._project_root / "Config" / "sources" / "active"
            )
        else:
            self._user_path = None

        # Legacy: explicit sources_dir overrides both paths
        if sources_dir is not None:
            self.sources_dir = Path(sources_dir)
        elif self._project_root is not None:
            # New mode: sources_dir points to user path (discovery
            # handles builtin separately)
            self.sources_dir = self._user_path
        else:
    ...
    def discover_sources(self) -> Dict[str, SourceConfig]:
        ...
        Scans builtin sources first, then user sources. User sources
        with the same name/ID as a builtin source take precedence.
        ...
        try:
            ...
            if self._builtin_path and self._builtin_path.exists():
                saved_sources_dir = self.sources_dir
                self.sources_dir = self._builtin_path
            ...
            if self._user_path and self._user_path.exists():
                ...
                self._discover_custom_sources()
                self.sources_dir = saved_sources_dir
            elif self.sources_dir and self.sources_dir.exists():
                # Legacy / fallback: single sources_dir
                self._discover_config_driven_sources()
                self._discover_custom_sources()
