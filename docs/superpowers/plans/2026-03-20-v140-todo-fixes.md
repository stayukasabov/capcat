...
| File | Change |
|------|--------|
| `capcat/core/source_system/add_source_command.py` | `execute()` returns `Path` instead of `None` |
| `capcat/core/source_system/add_source_service.py` | Capture return value; delete `_write_manifest_entry_after_add()` |
| `capcat/core/source_config_mirror.py` | `_load_manifest()` returns `None` when file absent; `check_for_upgrades()` checks `is None` |
| `capcat/cli.py` | Hoist `find_project_root()` to top of `_cmd_list()` |
| `docs/reference/quick-reference.md` | Add Source Management section |
| `tests/unit/test_add_remove_source_mirror.py` | Update/add tests for TODO 1 |
| `tests/unit/test_source_config_mirror.py` | Add tests for TODO 2 |
